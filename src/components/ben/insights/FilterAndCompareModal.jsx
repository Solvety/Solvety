  ...
  return (
    <div className="grid grid-cols-12 gap-2">
      {gridConfig.map((config, index) => (
        <button
          className={`font-semibold py-2 ${config.col} text-center py-0 px-4 rounded-lg ${
...
const FilterAndCompareModal = ({ open, handleClose, position }) => {
  ...
  return (
    ...
    >
      <Box
        className={`bg-white absolute top-[50%] ${position === 'left' ? 'left-[22%]' : 'right-[6%] -z-20'} w-[35%] rounded-xl shadow-xl -translate-y-1/2 mx-auto p-4`}
      >
...
export default FilterAndCompareModal;